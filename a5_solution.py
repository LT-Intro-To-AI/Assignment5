...
class Board:
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        """
        # set assignment value & update num_nums_placed
        self.rows[row][column] = assignment
        self.num_nums_placed += 1

        # in given row and column, remove assignment as a possibility
        for i in range(self.size):
            remove_if_exists(self.rows[row][i], assignment)
            remove_if_exists(self.rows[i][column], assignment)

        # remove the assignment from the possibilities in the subgrid
...
def DFS(state: Board) -> Board:
    ...
    while not the_stack.is_empty():
        ...
        if curr.goal_test():
            ...
            # if we haven't solved the board but reached an invalid state stop
            # attempting to fill it out (backtracking)
            pass

    # if this is reached then initial board state was malformed
    return None
